...
interface IAssignUserRole {
    userId: string;
    roleIds: string[];  // Change roleId to roleIds to store multiple role IDs
}

const initialState: IAssignUserRole = {
    userId: "",
    roleIds: []  // Initialize roleIds as an empty array
}
    ...
    useEffect(() => {
        if (editMode && userRoleData) {
            setAssignUserRole({
                ...
                roleIds: userRoleData.role_ids || []  // Assuming role_ids is an array in userRoleData
            });
        } else {
                        ...
                        <Col>
                            <Form.Group controlId="formRole" className='mb-2'>
                                ...
                                <Select
                                    ...
                                    value={roles?.filter(role => assignUserRole.roleIds.includes(role.id)).map(role => ({
                                        value: role.id,
                                        label: role.roleName
                                    }))}
